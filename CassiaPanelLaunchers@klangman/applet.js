...
class PanelAppLauncher extends DND.LauncherDraggable {
    ...
    workspaceChanged(wsWindows) {
       ...
       if ((this.dspNumber === DisplayNumber.All && this._curWorkspaceWindowCnt >= 1) || (this.dspNumber === DisplayNumber.Smart && this._curWorkspaceWindowCnt >= 2)) {
           this._labelNumber.set_text(this._curWorkspaceWindowCnt.toString());
    ...
    windowAdded(metaWindow, workspaceIdx) {
        //log( "PanelAppLauncher.windowAdded for "+this.app.get_name() );
        this._windows.push(metaWindow);
        this._curWorkspaceWindowCnt++;
        this.actor.add_style_pseudo_class("active");
        if (this.dspNumber === DisplayNumber.All || (this.dspNumber === DisplayNumber.Smart && this._curWorkspaceWindowCnt >= 2)) {
            this._labelNumber.set_text(this._curWorkspaceWindowCnt.toString());
        }
        if ((this.dspNumber === DisplayNumber.All && this._curWorkspaceWindowCnt === 1) || (this.dspNumber === DisplayNumber.Smart && this._curWorkspaceWindowCnt === 2)) {
           this._labelNumberBox.show();
    ...
    windowRemoved(metaWindow, workspaceIdx) {
        //log( "PanelAppLauncher.windowRemoved for "+this.app.get_name() );
    ...
    _updateIconSize() {
        //log( `CPL: pannelHeigth: ${this.launchersBox._panelHeight}  iconSize: ${this.icon.get_icon_size()}` );
        if (this.icon.get_icon_size() > this.launchersBox._panelHeight - 4)
           this.icon.set_icon_size(this.launchersBox._panelHeight - 4);
    }

   _updateOrientation() {
      //log( `in _updateOrientation for app ${this.app.get_name()} with ${this._curWorkspaceWindowCnt} windows` )
      //var err = new Error();
      //log( "Stack:\n"+err.stack );
...
class CinnamonPanelLaunchersApplet extends Applet.Applet {
    ...
    _windowAdded(screen, metaWindow, monitor) {
        ...
        if (launcher) {
           launcher.windowAdded(metaWindow, currentWs);
        }
    }
    ...
    _windowRemoved(screen, metaWindow, monitor) {
        ...
        //if (!launcher) log( "window launcher was not found for window: " + metaWindow );
        if (launcher) {
           launcher.windowRemoved(metaWindow, currentWs);
        }
    }
    ...
    on_orientation_changed(neworientation) {
       log( `Cur orientation ${this.orientation} new ${neworientation}` ); 
